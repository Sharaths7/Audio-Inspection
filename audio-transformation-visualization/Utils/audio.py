...
def action(file_uploader_1, file_uploader_2, selected_provided_file, transformations):
    if file_uploader_1 is not None:
        y1, sr1 = handle_uploaded_audio_file(file_uploader_1)
    else:
        pass
        # Handle default or selected file 1

    if file_uploader_2 is not None:
        y2, sr2 = handle_uploaded_audio_file(file_uploader_2)
    else:
        pass
        # Handle default or selected file 2

    pipeline_1 = audiomentations.Compose(create_pipeline(transformations))
    pipeline_2 = audiomentations.Compose(create_pipeline(transformations))  # You might want different transformations here

    st.header('Audio File 1')
    plot_audio_transformations(y1, sr1, pipeline_1)

    st.header('Audio File 2')
    plot_audio_transformations(y2, sr2, pipeline_2)

    # Here you can compare different aspects of y1 and y2, apply transformations and display the comparisons
    # You might want to add logic to handle the comparison of transformations applied to both files
